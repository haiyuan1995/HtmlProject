 ...
 *   然后在页面中调用  {@link #setImageResources(ArrayList, String[] ,ImageCycleViewListener) }即可!
 *
 *   另外提供{@link #startImageCycle()
 *   @link #pushImageCycle() 两种方法，用于在Activity不可见之时节省资源；
...
public class ImageCycleView extends LinearLayout {
	 ...
	 */
	public void setImageResources(ArrayList<String> imageUrlList,String[] strings, ImageCycleViewListener imageCycleViewListener) {
		// 清除所有子视图
		...
		mtext.setPadding(120, 0, 0, 0);
		mtext.setTextColor(Color.WHITE);
		mtext.setText(strings[0]);
		mGroup.addView(mtext);
		...
		for (int i = 0; i < imageCount; i++) {
			...
			int imageParams = (int) (mScale * 8 + 0.5f);// px与DP转换，适应不同分辨率
		...
		mAdvPager.setPageTransformer(true, new DepthPageTransformer());
		mAdvPager.setAdapter(mAdvAdapter);
        mAdvPager.setOffscreenPageLimit(4);//设置缓存个数
		startImageTimerTask();
